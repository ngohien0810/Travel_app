...
const transition = (durationMs = DURATION) => (
    <Transition.Together>
        <Transition.Out type="fade" interpolation="easeInOut" durationMs={durationMs} />
        <Transition.Change interpolation="easeInOut" durationMs={durationMs} />
        <Transition.In type="fade" durationMs={durationMs} interpolation="easeOut" />
    </Transition.Together>
);
...
export const PostDelay = ({ children, durationMs }: PostDelayProps) => {
    ...
    // render
    return (
        <Transitioning.View style={[styles.container]} transition={transition(durationMs)} ref={postDelayViewRef}>

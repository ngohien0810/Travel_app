...
const ItemCommentComponent = ({ index, overlayColor }: ItemCommentProps) => {
    // state
    const widthComment = useMemo<number>(() => MIN_WIDTH_ITEM + Math.random() * (MAX_WIDTH_ITEM - MIN_WIDTH_ITEM), []);
    ...
    // effect
    useEffect(() => {
        progress.value = withDelay(index * DELAY_MS, sharedTiming(1, { duration: DURATION }));

...
import { LayoutChangeEvent, LayoutRectangle, StyleProp, Text, View, ViewStyle } from 'react-native';
...
export const HelperText = ({ msg, type, colorThemeInfo, colorThemeError, visible = false }: HelperTextProps) => {
    // state
    ...
    const textStyle = useMemo<StyleProp<ViewStyle>>(
        () => [
            ...
            type === 'error'
                ? {
                      color: colorThemeError ? theme.colors[colorThemeError] : ColorDefault.error,
                  }
                : {
                      color: colorThemeInfo ? theme.colors[colorThemeInfo] : ColorDefault.info,
                  },
        ],
    ...
    return (
        <View style={[styles.container]}>
            <Animated.View pointerEvents={'none'} onLayout={_onLayoutContent} style={[styles.hiddenView]}>

...
import { LayoutChangeEvent, NativeSyntheticEvent, TextInput, TextInputChangeEventData, View } from 'react-native';
...
export const InputFlat = forwardRef<any, InputFlatProps>((props, ref) => {
    ...
    const bottom = useInterpolate(progress, [0, 1], [0, heightContainerInput - 10]);
    ...
    // string
    const labelText = useMemo(() => (labelT18n && t(labelT18n)) || label || undefined, [labelT18n, label, t]);
            ...
            <View style={[styles.content]}>
                {(placeholderT18n || placeholder) && value.length === 0 && (
                    <View style={[styles.wrapPlaceHolder]} pointerEvents={'none'}>
                        <Text t18n={placeholderT18n} text={placeHolder} color={placeholderColor} />
                    </View>
                )}
                {labelText && (

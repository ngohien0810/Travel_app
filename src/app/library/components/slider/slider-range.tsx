...
import { sharedClamp, sharedMax, sharedSub, sharedTiming, useInterpolate, useMin } from '@animated';
import { onCheckType } from '@common';

import { FIXED_AFTER, INITIAL_RANGE, LOWER_BOUND, THUMB_SIZE, UPPER_BOUND } from './constants';
    ...
    (
        ...
        const runAnimation = useCallback(
            (lowerValue: number, upperValue: number) => {
                const percentLeft = (lowerValue - lowerBound) / (upperBound - lowerBound);
                const percentRight = (upperValue - lowerBound) / (upperBound - lowerBound);
        ...
        useImperativeHandle(
            ...
            () => ({
                setRange: (args: ArgsChangeRange) => {
                    ...
                    runAnimation(args.reverted ? args.upper : args.lower, !args.reverted ? args.upper : args.lower);
                },
            }),
            [runAnimation]
        );

...
const _callActionListeners = (action: Action, listenerContainer: ActionListenerContainer[]) => {
    for (let i = listenerContainer.length - 1; i >= 0; i--) {
        const listener = listenerContainer[i];
        if (action && onCheckType(action.type, 'string') && listener.action === action.type) {

...
export const useSharedTransition = (
    ...
    return useDerivedValue(() =>
        withTiming(value.value, Object.assign({ duration: 500, easing: Easing.bezier(0.33, 0.01, 0, 1) }, config))
    );
};
 ...
 */
export const useSharedSpringTransition = (state: boolean, config?: WithSpringConfig): Animated.SharedValue<number> => {
    const value = useSharedValue(0);
    useEffect(() => {
        value.value = typeof state === 'boolean' ? sharedBin(state) : state;
    }, [state, value]);

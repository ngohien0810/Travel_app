 ...
 */
export const useShareClamp = (value: Animated.SharedValue<number>, lowerValue: number, upperValue: number) => {
    'worklet';
...
export const useTiming = ({ callback, config, from = 0, toValue = 1, delay = 0 }: UseTimingParams = {}) => {
...
export const useSpring = ({ callback, config, from = 0, toValue = 1, delay = 0 }: UseSpringParams = {}) => {
    const progress = useSharedValue(from);

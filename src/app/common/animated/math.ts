 ...
 */
export const sharedClamp = (value: number, lowerValue: number, upperValue: number) => {
    'worklet';
...
export const sharedSub = (...args: number[]) => {
    ...
    return args.slice(1).reduce((accumulator, curr) => accumulator - curr, args[0]);
};
 ...
 */
export const SharedSnapPoint = (value: number, velocity: number, points: number[]) => {
    'worklet';
